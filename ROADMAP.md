...
| A3 | Deploy einrichten | ✅ | **Direct Upload statt Git-Integration** (Dashboard-Git-Flow war 2026 unbrauchbar versteckt): Wrangler CLI (`wrangler login` einmalig). Live: https://geo-quiz-a6s.pages.dev — Achtung: Push deployt NICHT automatisch. Seit 2026-07-22 macht **`npm run release`** (`frontend/scripts/release.mjs`) Web-Deploy **und** signierten App-Build aus demselben `dist/`; `npm run deploy` bleibt als reiner Web-Upload bestehen. Details: [docs/DEVELOPMENT.md §11](docs/DEVELOPMENT.md) |
...
| B5 | Emulator-Test: alle Modi, Fokus Karten (Pan/Zoom/Tap-Präzision, Stress-Test kleine Länder wie Luxemburg) | ✅ | AVD `geoquiz_pixel7` (Pixel 7, Android 14) eingerichtet, APK installiert; Flaggen + Profil + Online-Anmeldung getestet. Erstes Pin-Modus-Feedback führte zum Vollbild-Umbau (siehe Verlauf 2026-07-11); Portrait + Landscape im Emulator verifiziert. `frontend/.env.local` mit neuem `sb_publishable`-Key wiederhergestellt. Vom Nutzer abgenommen (2026-07-18) |
| B6 | Test auf echtem Gerät (`npx cap run android`) | ⬜ | braucht USB-Debugging |
| B7 | App-Icon + Splash-Screen im 8-Bit-Look | ✅ | Pixel-Globus (Ozean = PICO-8-Blau, Land = Grün wie das Wordmark) + „GEOQUIZ"-Schriftzug, prozedural erzeugt via `frontend/scripts/generate-app-assets.mjs` (eigener PNG-Encoder, keine Deps) → `frontend/assets/` → `@capacitor/assets generate --android` (74 Dateien). Android-12-Systemsplash auf Navy (`windowSplashScreenBackground`), Launcher-Fallback-Farbe ebenfalls. Im Emulator verifiziert (App-Drawer-Icon + Kaltstart) |
| B8 | Signierter Release-Build (`.aab`/`.apk`) | ✅ | Keystore vom Nutzer angelegt (dediziertes Passwort, liegt außerhalb des Repos in `C:\Users\<user>\keystores\`); Gradle liest Pfad+Passwörter aus gitignorter `android/keystore.properties` (Template: `keystore.properties.example`), ohne Datei baut Release unsigniert weiter. `assembleRelease` + `bundleRelease` erfolgreich, Signatur per `apksigner verify` bestätigt (CN=Tobias Sobek). Play-Store-Upload weiter optional/später |
| B9 | Feedback-Runde 1 aus dem App-Test: System-Back → Menü, Login lädt Fortschritt, Choice ohne Scrollen, Pin-Overlay aufgeräumt | ✅ | Alle 4 Punkte umgesetzt ([DESIGN-MOBILE-POLISH.md](DESIGN-MOBILE-POLISH.md)); Punkte 2–4 wirken auch im Web. Browser-verifiziert (360×640 + Desktop), Android-Debug-Build grün — **Bestätigung auf dem Gerät offen (Back-Button + Login)** |
...
| H4 | Migrationen 0010–0012 auf Live-DB einspielen | ✅ | vom Nutzer eingespielt (2026-07-16) — fremde Avatare, Cup-Aufklapp-Button und echte fremde Spielerkarten erscheinen jetzt |
...
| H8 | Beta-Tester-Avatar „Bug" (Marienkäfer), zeitlich befristet bis 31.08.2026 (Nutzer-Wunsch) | ✅ | Migration `0015_beta_tester_avatar.sql` (`player_stats.beta_tester`, gesetzt in `submit_score`/`submit_cup_run`, rückwirkender Backfill); neuer Avatar-Unlock-Kind `betaTester`. Live-DB bestätigt (2026-07-18) — **Phase H damit komplett abgeschlossen** |
...
| Datum | Eintrag |
|---|---|
| 2026-07-22 | **Umbenannt in „GEOQUIZ ARCADE"** ([DESIGN-RENAME.md](DESIGN-RENAME.md)) — bewusst vor dem ersten Play-Store-Upload, weil die `applicationId` danach nie wieder änderbar ist. Sichtbarer Name (`appName`, `app_name`, `<title>`, Gruppen-Einladung) + Package-ID `de.tobsob.geoquiz` → `de.tobsob.geoquizarcade` (capacitor.config.ts, build.gradle `namespace`+`applicationId`, `MainActivity.java` samt Verzeichnis, strings.xml `package_name`+`custom_url_scheme`). Splash-Wordmark jetzt zweizeilig (GEO grün / QUIZ cyan / ARCADE weiß), Font um die Glyphen A R C D erweitert, `drawLines()` für Mehrzeiligkeit — das **Icon bleibt unverändert**, es trägt nur den Globus. Verifiziert: `aapt dump badging` zeigt `de.tobsob.geoquizarcade` + Label „GEOQUIZ ARCADE", `assembleDebug` grün, Web-Build grün mit neuem `<title>`. Dazu auf Nutzer-Idee ein **Attract-Mode im Header** (`components/Wordmark.tsx`): blendet im Leerlauf zwischen „GEOQUIZ" (grün/cyan) und „ARCADE" (gelb) hin und her wie ein Automaten-Titelbildschirm, reines CSS ohne Timer. Auf dem **Splash geht das nicht** (statisches Bitmap-Drawable, Android-12-AnimatedVector nur im Icon-Slot mit ~1 s Deckel) — dort bleibt der Zweizeiler. Bewusst nur im Leerlauf: auf `/play/*`, `/cup`, `/training` steht es still auf „GEOQUIZ", weil unter Zeitdruck nichts neben der Frage blinken darf; `prefers-reduced-motion` schaltet es ganz ab; beide Wörter in derselben Grid-Zelle, damit die Kopfzeile nicht springt. Im Browser verifiziert: Deckkräfte ergänzen sich jederzeit zu 1 (kein leerer Header), Container-Breite = Breite von GEOQUIZ (kein Springen), `wordmark--frozen` greift auf `/training`. Bewusst NICHT umbenannt: localStorage-Keys (`geo-quiz-*`, sonst Fortschrittsverlust), Repo, Cloudflare-Projekt/Live-URL, Migrationen. **Release raus** — `npm run release` durchgelaufen, Produktion liefert den neuen Bundle (`index-BlV840wz.js`), Tab-Titel „GEOQUIZ ARCADE", Wordmark-Attract-Mode aktiv, Konsole sauber; signierte `GeoQuizArcade-1.0-1-2026-07-22.apk` (12,9 MB) + `.aab` (12,7 MB) in `frontend/release/`, Signatur `CN=Tobias Sobek`. **Offen (manuell):** alte Installation deinstallieren (neue Package-ID = andere App). **Kein Supabase-Schritt nötig** — zwischenzeitlich falsch behauptet, dann geprüft: `custom_url_scheme` ist ungenutztes Capacitor-Gerüst, der OAuth-Rücksprung geht über `origin + pathname` (`https://localhost/` in der WebView), und das Manifest hat nur den LAUNCHER-Intent-Filter, keinen Deep-Link |
| 2026-07-22 | **Ein Release-Befehl für beide Plattformen** (Nutzer-Frage „macht `npm run deploy` auch eine neue APK?" — nein, tat es nicht): `frontend/scripts/release.mjs` + `npm run release` fährt Tests + Lint → Vite-Build → Cloudflare Pages → `cap sync` → Gradle `assembleRelease`/`bundleRelease`, sammelt die Artefakte mit Versions-/Datumsnamen in `frontend/release/` (gitignored) und bestätigt die Signatur per `apksigner`. Kernpunkt: **derselbe** `dist/`-Build geht ins Web und in die App, Web und App können nicht mehr auseinanderlaufen. Varianten `release:web` / `release:android` / `--no-deploy` / `--debug-apk` / `--skip-checks`; Vorprüfung auf `JAVA_HOME`/`ANDROID_HOME`/`keystore.properties` mit klarer Warnung statt stiller unsignierter APK. Verifiziert: kompletter Lauf mit `--no-deploy` grün (132 Tests, signierte APK 12,8 MB + AAB 12,6 MB, `CN=Tobias Sobek`). `npm run deploy` bleibt als reiner Web-Upload. Nachgereicht nach dem ersten Nutzer-Versuch: **Root-`package.json` als Skript-Durchreiche** — `npm run release` im Repo-Root scheiterte sonst mit ENOENT, weil die `package.json` in `frontend/` liegt. Doku: [docs/DEVELOPMENT.md §11](docs/DEVELOPMENT.md) |
| 2026-07-22 | Bestenliste auf **Kalender-Zeiträume** umgebaut (Nutzer-Feedback „die Ansichten sind nicht konsequent, sie zeigen nicht die aktuelle Woche", [DESIGN-LEADERBOARD-PERIODS.md](DESIGN-LEADERBOARD-PERIODS.md)): „Woche/Monat/Jahr" waren rollierende Fenster (letzte 7/30/365 Tage), die Pokale dagegen Kalenderperioden in Europe/Berlin — dasselbe Wort meinte zwei verschiedene Mengen. Jetzt beidseitig Kalenderwoche (Mo–So)/-monat/-jahr, plus ◀/▶-Blättern mit derselben Beschriftung wie im Pokale-Tab („KW 30 2026"), ▶ in der laufenden Periode gesperrt („läuft noch"), ◀ an der ältesten Periode mit Einträgen. Neu: `features/leaderboard/periods.ts` (+19 Tests), Migration `0016` (`p_until` additiv + `get_leaderboard_first_played`), Client fällt ohne 0016 sauber auf die laufende Periode zurück. Tests 132/132, tsc/lint/Build grün, im Browser durchgeklickt (KW 30→29→28→27 mit korrekten Sperren, Monats-/Jahreswechsel, Cups-Tab). **0016 noch am selben Tag vom Nutzer eingespielt** (RPC-Probe bestätigt: `p_until` + `get_leaderboard_first_played` vorhanden, anon korrekt gesperrt) — `apply_pending.sql` wieder gelöscht, Live-DB-Stand 0001–0016. **Release raus** — `npm run release` durchgelaufen, Produktion liefert den neuen Bundle (`index-DF1gjkKx.js`), Seite lädt ohne Konsolenfehler; signierte APK/AAB in `frontend/release/`. Merker: die Produktions-URL zeigte ~1 min nach dem Deploy noch den alten Bundle — reine Cache-Propagation, kein Fehler; vor dem Nachbohren einfach kurz warten oder mit Cache-Buster prüfen |
| 2026-07-19 | App-Feedback-Runde 1 umgesetzt (B9, [DESIGN-MOBILE-POLISH.md](DESIGN-MOBILE-POLISH.md)): (1) Android-System-Back führt via `@capacitor/app`-Listener zurück ins Menü statt die App zu schließen (nur Home beendet); (2) **Login-Bug behoben** — E-Mail-Anmeldung setzte nur den Namen, Level/XP/Abzeichen/Avatar luden erst beim nächsten App-Start (betraf auch Web); jetzt zentraler `applyAuthSession()`-Helfer für App-Start/Login/Registrierung/Logout; (3) Choice-Modi passen auf 360×640 ohne Scrollen (2×2-Antwort-Grid, dvh-gedeckelte Grafik, gestraffte Abstände); (4) Pin-Modus: Zoom-Buttons mobil ausgeblendet, OSM/CARTO-Attribution als einklappbares ⓘ unten rechts über der Action-Bar. Tests 113/113, tsc/lint/Build grün, Browser-verifiziert mobil + Desktop, `assembleDebug` grün. Geräte-Bestätigung (Back-Button, Login-E2E) steht aus |
| 2026-07-18 | B5 vom Nutzer abgenommen („sieht gut aus") + B8 umgesetzt: Release-Signierung über gitignorte `keystore.properties` (Keystore + dediziertes Passwort bleiben komplett lokal beim Nutzer, Fallback baut unsigniert), signiertes APK + AAB gebaut, Signatur verifiziert. Offen in Phase B nur noch B6 (Test auf echtem Gerät) |
| 2026-07-18 | B7 umgesetzt: App-Icon + Splash-Screen im 8-Bit-Look — Pixel-Globus + GEOQUIZ-Wordmark, prozedural als Code (`frontend/scripts/generate-app-assets.mjs`, eigener PNG-Encoder wie beim Avatar-Ansatz „Grafik als Code"), alle Android-Dichten via `@capacitor/assets`. Emulator-Verifikation davor: App läuft mit allen neuen Features (Icons, Auth-Panels, Avatar-Picker) sauber im Android-Emulator (B5-Zwischenstand) |
| 2026-07-18 | Migration 0015 vom Nutzer auf der Live-DB eingespielt — Beta-Tester-Avatar „Bug" ist damit live scharf. `apply_pending.sql` gelöscht (nichts mehr ausstehend), Live-DB-Stand 0001–0015 |
| 2026-07-18 | Zwei Bug-Fixes + neuer Beta-Tester-Avatar (Nutzer-Reports/-Wunsch): eigene Spielerkarte zeigte Bestpunkte aus dem lokalen `progressStore` statt der Server-RPC (kleinere Werte als die echte Bestleistung) — behoben, lädt jetzt wie fremde Karten über `get_player_card`. Pin-Modus-Streak-Badge war unsichtbar für Bruchteil-Streaks (0,1/0,5) — Schwelle von `< 1` auf `<= 0` korrigiert. Neuer Marienkäfer-Avatar „Bug" als zeitlich befristetes Dankeschön für alle, die bis 31.08.2026 spielen (Migration 0015, Details in [DESIGN-AVATARS.md](DESIGN-AVATARS.md) V11) |
