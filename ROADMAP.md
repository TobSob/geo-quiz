...
| H8 | Beta-Tester-Avatar „Bug" (Marienkäfer), zeitlich befristet bis 31.08.2026 (Nutzer-Wunsch) | ✅ (Code) | Migration `0015_beta_tester_avatar.sql` (`player_stats.beta_tester`, gesetzt in `submit_score`/`submit_cup_run`, rückwirkender Backfill); neuer Avatar-Unlock-Kind `betaTester`. **Auf Live-DB noch einspielen** (`apply_pending.sql`) |
