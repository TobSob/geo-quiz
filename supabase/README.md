...
> Live-DB-Stand: **0001–0009 vollständig eingespielt** (verifiziert 2026-07-14) — **0010–0012 stehen noch aus** (bis dahin läuft der Client graceful: keine fremden Avatare, Cup-Score ohne Aufklapp-Button, fremde Karten zeigen „gerade nicht verfügbar"). `apply_all.sql` dient der Vollinstallation frischer Projekte — auf der bestehenden Live-DB nichts erneut ausführen (0005 löscht Scores, der 0008-Backfill würde XP doppeln!); für die Live-DB einfach die Inhalte von `0010_profile_avatars.sql`, `0011_cup_leg_breakdown.sql` und `0012_player_card.sql` im SQL-Editor ausführen.
