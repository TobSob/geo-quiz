...
| `0016_calendar_leaderboard_periods.sql` | **Kalender-Zeiträume in der Bestenliste (Nutzer-Feedback):** `get_leaderboard_scores`/`get_leaderboard_cups` bekommen ein zusätzliches, defaultetes `p_until` → halboffenes Fenster `[p_since, p_until)`; die Periodengrenzen (Kalenderwoche Mo–So / -monat / -jahr in Europe/Berlin, deckungsgleich mit den Pokal-Perioden) rechnet der Client. Neu: `get_leaderboard_first_played(mode, group)` als Untergrenze fürs ◀-Blättern. Rein additiv — ein alter Client (nur `p_since`) läuft unverändert weiter. Regelwerk: [DESIGN-LEADERBOARD-PERIODS.md](../DESIGN-LEADERBOARD-PERIODS.md) |

| `0017_account_deletion.sql` | **Konto-Löschung (Play-Pflicht, [DESIGN-PLAYSTORE.md](../DESIGN-PLAYSTORE.md)):** `delete_own_account()` — `security definer`, **ohne Parameter**, löscht ausschließlich `auth.users where id = auth.uid()`. Alles Weitere fällt per `on delete cascade` (alle 12 Nutzertabellen hängen daran), deshalb bewusst keine Tabellenliste, die beim nächsten Feature veraltet. `grant` nur an `authenticated` — das schließt Gäste ein (anonyme Sitzungen haben dieselbe Rolle mit `is_anonymous:true`) |

> Live-DB-Stand: **0001–0017 vollständig eingespielt** (0001–0009 verifiziert 2026-07-14, 0010–0015 vom Nutzer 2026-07-16/18, 0016 am 2026-07-22, **0017 am 2026-08-04**). Nichts mehr ausstehend. `apply_all.sql` dient der Vollinstallation frischer Projekte — auf der bestehenden Live-DB nichts erneut ausführen (0005 löscht Scores, der 0008-Backfill würde XP doppeln!).
>
> 0017 gegen die Live-DB verifiziert (2026-08-04): Aufruf mit blossem Anon-Key → `42501 permission denied` (die Funktion existiert und `anon` ist ausgesperrt; fehlte sie, käme `PGRST202`). Danach E2E mit einem Wegwerf-Gastkonto: anonyme Anmeldung → Profil angelegt und lesbar → `delete_own_account()` → HTTP 204 → `/auth/v1/user` antwortet `user_not_found`, Profilzeile per Cascade weg. 6/6 Prüfungen.
